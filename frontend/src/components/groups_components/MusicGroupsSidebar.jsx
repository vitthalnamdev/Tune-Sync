...
import { Headphones, Users, Music, ChevronDown, ChevronRight, User, Plus, Play, Pause } from 'lucide-react';

const MusicGroupsSidebar = () => {
        ...
        {groups.map(group => (
          <div key={group.id} className="border-b border-gray-700">
            ...
            >
              ...
              <div className="flex items-center">
                {group.isPlaying && <div className="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></div>}
                  ...
                  {group.members.map(member => (
                    ...
                    >
                      <div className="relative mr-3">
                        ...
                        <div 
                          className={`absolute -bottom-1 -right-1 w-2 h-2 rounded-full ${member.online ? 'bg-green-500' : 'bg-gray-500'}`}
                        />
                      </div>
                      <div className="flex-1">
                        <span className="text-sm">{member.name}</span>
                        {member.host && <span className="ml-2 text-xs text-purple-400">(Host)</span>}
                      </div>
                    </div>
                  ))}
                </div>
