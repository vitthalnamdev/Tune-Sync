    ...
    <div className="min-h-screen box-content bg-black flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {Array.from({ length: 20 }).map((_, i) => (
          <div 
            key={i} 
            className={`absolute rounded-full bg-blue-${Math.floor(Math.random() * 3) + 4}00 opacity-${Math.floor(Math.random() * 3) + 1}0`}
            style={{
              top: `${Math.random() * 100}%`,
      ...
      <div className={`absolute -top-60 -right-60 w-96 h-96 bg-gradient-to-br from-indigo-900 to-purple-800 rounded-full filter blur-3xl opacity-20 transition-all duration-1000 ${isLoaded ? 'translate-y-10' : ''}`}></div>
      <div className={`absolute top-20 -left-40 w-80 h-80 bg-gradient-to-tr from-blue-900 to-cyan-800 rounded-full filter blur-3xl opacity-20 transition-all duration-1000 delay-300 ${isLoaded ? 'translate-x-16' : ''}`}></div>
      <div className={`absolute bottom-10 right-20 w-72 h-72 bg-gradient-to-tl from-purple-900 to-pink-800 rounded-full filter blur-3xl opacity-20 transition-all duration-1000 delay-700 ${isLoaded ? 'translate-y-0' : 'translate-y-20'}`}></div>
      
      <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black opacity-40 z-0"></div>
      
      <div className={`sm:mx-auto sm:w-full sm:max-w-md z-10 transition-all duration-700 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
        <div className="flex justify-center">
          <div className="relative h-20 w-20 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 p-4 shadow-lg shadow-blue-500/30 hover:shadow-blue-500/40 transition-all duration-300 overflow-hidden group">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-full w-full text-white relative z-10">
      ...
      <div className={`mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 transition-all duration-700 delay-200 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
        <div className="bg-gray-900 bg-opacity-80 backdrop-blur-lg py-8 px-4 shadow-2xl shadow-indigo-500/10 sm:rounded-2xl sm:px-10 border border-gray-800 relative overflow-hidden">
          ...
          {isSuccess ? (
            <div className={`text-center transition-all duration-500 ${isSuccess ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
              <div className="relative rounded-full h-20 w-20 flex items-center justify-center mx-auto overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-green-400 to-cyan-500 animate-pulse opacity-50"></div>
                <div className="bg-gradient-to-r from-green-400 to-cyan-500 rounded-full h-16 w-16 flex items-center justify-center relative">
                  <svg className="h-8 w-8 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                  </svg>
                </div>
              </div>
              <h3 className="mt-4 text-2xl font-bold bg-gradient-to-r from-cyan-400 to-green-400 bg-clip-text text-transparent">Authentication Successful</h3>
              <p className="mt-2 text-gray-300">Welcome back to the system</p>
              <button 
                className="mt-6 w-full flex justify-center py-3 px-4 border-0 rounded-xl text-base font-medium text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 focus:outline-none shadow-lg shadow-blue-700/30 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98]"
            ...
            <form className="space-y-6 relative z-10" onSubmit={handleSubmit}>
              ...
              <div className={`transition-all duration-500 delay-200 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <div className="flex items-center justify-between">
                  ...
                  <a href="#" className="text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors duration-150 hover:underline">
                    Forgot password?
                  </a>
                </div>
              ...
              <div className={`flex items-center transition-all duration-500 delay-300 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <label className="inline-flex items-center cursor-pointer">
                  ...
                  <span className="ml-3 text-sm font-medium text-gray-300">Remember me</span>
                </label>
              </div>

              <div className={`pt-2 transition-all duration-500 delay-400 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <button
                  type="submit"
              ...
              <div className={`mt-6 transition-all duration-500 delay-500 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <p className="text-center text-sm text-gray-400">
                  Don't have an account?{' '}
                  <a href="/signup" className="font-medium bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent hover:from-cyan-300 hover:to-blue-400 transition-colors duration-150">
                    Create one now
                  </a>
                </p>
              </div>
              
              <div className={`flex items-center justify-center space-x-4 mt-6 pt-6 border-t border-gray-800 transition-all duration-500 delay-600 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <button
                  type="button"
